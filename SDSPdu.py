...
class SDSPdu:
    ...
    The current format for the PDU is Message_type,
    ...
    def send(self, message_type, data):
        print('data :{} message_type: {}'.format(data, message_type))
        try:
            data = data.decode()
        except AttributeError:
            pass
        ...
        if data_arr is not None:
            ...
            for d in data_arr:
                pdu = self.generate_byte(message_type, d)  # returns the pdu in byte
                print('sending')
                print(pdu)
